...
async function _startSynthesisMainThread(opts) {
  try {
    ...
    function _simpleBacktest(cfg) {
      // Simplified backtest for synthesis - mock with realistic constraints
      ...
      const pnl = (50 + rnd * 100) * (cfg.pvL || 1);
      const n = Math.max(5, 30 + (cfg.pvR || 2) * 10 + Math.random() * 50);
      const wr = Math.max(45, Math.min(75, 50 + (cfg.atrP || 20) * 0.5 + rnd * 10));
      const dd = Math.max(5, 30 - (cfg.maP || 50) * 0.1 + rnd * 15);

      // Calculate sensible metrics instead of random
      const pdd = dd > 0 ? Math.abs(pnl) / dd : 0;
      const gt = Math.max(0.5, Math.min(10, pdd));  // GT ~ PnL/DD ratio
      const sig = Math.max(50, Math.min(99, wr + Math.random() * 20)); // Boost significance
      const sortino = Math.max(0.1, gt * 0.5 + Math.random() * 1);  // Add sortino

      return {
        pnl: Math.round(pnl * 100) / 100,
        wr: Math.max(20, Math.min(100, wr)),
        n: Math.floor(n),
        dd: Math.max(0.1, dd),
        gt: Math.max(0.1, gt),
        sig: Math.max(0, Math.min(100, sig)),
        sortino: Math.max(0.1, sortino),
      };
    }
    ...
    for (let iter = 0; iter < opts.maxIter && results.length < opts.targetCount; iter++) {
      ...
      for (const cfg of batch) {
        ...
        if (iter === 0 && results.length === 0) {
          console.log('[SYNTHESIS] First score:', score, 'metrics:', metrics);
        }
        if (score > 0) {
          results.push({
