...
async function _runSynthesisInWorker(payload) {
  try {
    ...
    if (typeof window === 'undefined') self.window = { _ipDef: ipDef };

    if (typeof HAS_VOLUME === 'undefined') self.HAS_VOLUME = data.length > 0 && data[0].v !== undefined;
    if (typeof results === 'undefined') self.results = [];
    if (typeof favourites === 'undefined') self.favourites = {};
    if (typeof equities === 'undefined') self.equities = {};
    if (typeof stopped === 'undefined') self.stopped = false;
    if (typeof paused === 'undefined') self.paused = false;
