...
function generatePineScript(r, mode = 'indicator') {
  ...
  lines.push(`use_climax  = input.bool(${b(c.useClimax && c.clxMode !== 'any')}, "Climax выход (только в плюс)", group=grp_exit)`);
  lines.push(`use_clx_any = input.bool(${b(c.useClimax && c.clxMode === 'any')}, "Climax± (любой)", group=grp_exit)`);
  ...
  lines.push(`use_dynsl_struct = input.bool(${b(c.useDynSLStruct || false)}, "Сжимать SL при нарушении структуры", group=grp_dynsl)`);
  lines.push(`dynsl_struct_mult = input.float(${f(c.dynSLStructMult || 0.3, 1)}, "Коэффициент сжатия SL", minval=0.1, maxval=0.9, step=0.1, group=grp_dynsl, active=use_dynsl_struct, tooltip="0.3 = SL × 0.3 при нарушении структуры")`);
  lines.push(``);
  ...
  lines.push(`use_wait    = wait_bars > 0 or wait_ret  // Вычисляется динамически, как в JS`);
  ...
  lines.push(`// ── Three White Soldiers / Three Black Crows ──`);
  lines.push(`bool soldiers_l = use_soldiers and close[3] > open[3] and close[2] > open[2] and close[1] > open[1] and open[2] >= open[3] and open[2] <= close[3] and open[1] >= open[2] and open[1] <= close[2] and close[2] > close[3] and close[1] > close[2]`);
  lines.push(`bool soldiers_s = use_soldiers and close[3] < open[3] and close[2] < open[2] and close[1] < open[1] and open[2] <= open[3] and open[2] >= close[3] and open[1] <= open[2] and open[1] >= close[2] and close[2] < close[3] and close[1] < close[2]`);
  lines.push(``);

  lines.push(`bool pat_l = pivot_l or (use_engulf and bull_engulf[1]) or (use_pinbar and bull_pin[1]) or donch_l or boll_l or atr_bo_l or (use_ma_touch and ma_touch_l) or tl_touch_l or tl_break_l or flag_l or tri_l or rsix_l or macr_l or free_l or macd_l or stx_l or volmv_l or inb_l or nrev_l or st_l or sqz_l or kalman_x_l or pchg_l or soldiers_l`);
  lines.push(`bool pat_s = pivot_s or (use_engulf and bear_engulf[1]) or (use_pinbar and bear_pin[1]) or donch_s or boll_s or atr_bo_s or (use_ma_touch and ma_touch_s) or tl_touch_s or tl_break_s or flag_s or tri_s or rsix_s or macr_s or free_s or macd_s or stx_s or volmv_s or inb_s or nrev_s or st_s or sqz_s or kalman_x_s or pchg_s or soldiers_s`);
  lines.push(``);
  ...
  lines.push(`    // Пересчитываем адаптивные множители на каждом баре`);
  lines.push(`    float sl_adaptive_mult_local = 1.0`);
  lines.push(`    if use_adaptive_sl`);
  lines.push(`        float atr_sl_curr = ta.atr(sl_atr_len)`);
  lines.push(`        float atr_sl_avg_local = ta.sma(atr_sl_curr, sl_atr_len * 2)`);
  lines.push(`        if atr_sl_avg_local > 0`);
  lines.push(`            float atr_sl_ratio_local = atr_sl_curr / atr_sl_avg_local`);
  lines.push(`            sl_adaptive_mult_local := 1.0 - sl_atr_mult * 0.5 + sl_atr_mult * 0.5 * math.sqrt(atr_sl_ratio_local)`);
  ...
  lines.push(`        cnt += 1`);
  lines.push(`        sl_atr_lvl := dir == 1 ? ep - ac * sl_atr_val * sl_adaptive_mult_local : ep + ac * sl_atr_val * sl_adaptive_mult_local`);
  lines.push(`    if use_sl_pct`);
  lines.push(`        cnt += 1`);
  lines.push(`        sl_pct_lvl := dir == 1 ? ep * (1 - sl_pct_val * sl_adaptive_mult_local/100) : ep * (1 + sl_pct_val * sl_adaptive_mult_local/100)`);
  ...
  lines.push(`    // Пересчитываем адаптивные множители на каждом баре`);
  lines.push(`    float tp_adaptive_mult_local = 1.0`);
  lines.push(`    if use_adaptive_tp`);
  lines.push(`        float atr_tp_curr = ta.atr(tp_atr_len)`);
  lines.push(`        float atr_tp_avg_local = ta.sma(atr_tp_curr, tp_atr_len * 2)`);
  lines.push(`        if atr_tp_avg_local > 0`);
  lines.push(`            float atr_tp_ratio_local = atr_tp_curr / atr_tp_avg_local`);
  lines.push(`            tp_adaptive_mult_local := 0.5 + 0.5 * math.sqrt(atr_tp_ratio_local)`);
  lines.push(`    int cnt = 0`);
  lines.push(`    if use_tp_atr`);
  lines.push(`        cnt += 1`);
  lines.push(`        tp_atr_lvl := dir == 1 ? ep + ac * tp_atr_val * tp_adaptive_mult_local : ep - ac * tp_atr_val * tp_adaptive_mult_local`);
  lines.push(`    if use_tp_pct`);
  lines.push(`        cnt += 1`);
  lines.push(`        tp_pct_lvl := dir == 1 ? ep * (1 + tp_pct_val * tp_adaptive_mult_local/100) : ep * (1 - tp_pct_val * tp_adaptive_mult_local/100)`);
  ...
  if (mode === 'strategy') {
    ...
    lines.push(`            if be_offset >= be_trig`);
    lines.push(`                // Ветка 1: beOff >= beTrig → немедленный выход на уровне beTrig (как в JS)`);
    lines.push(`                float _be_exit = s_ep + s_dir * _u * be_trig`);
    lines.push(`                if s_dir == 1`);
    lines.push(`                    strategy.exit("LX", "L", limit=_be_exit, stop=_be_exit)`);
    lines.push(`                else`);
    lines.push(`                    strategy.exit("SX", "S", limit=_be_exit, stop=_be_exit)`);
    lines.push(`            else`);
    lines.push(`                // Ветка 2: beOff < beTrig → перестановка SL на уровень beOffset`);
  ...
  lines.push(`        table.cell(t, 0, 16, kelly_txt + adapt_txt, bgcolor=color.new(color.aqua,40), text_color=color.white, text_size=table_sz)`);
