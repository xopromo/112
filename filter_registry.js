...
const FILTER_REGISTRY = [
  ...
  // Блокирует если MA(equity) не растёт (falling or flat)
  {
    ...
    blocksL:  (cfg, i) => {
      ...
      if (ma <= 0) return false;  // warmup — фильтр не влияет, входы разрешены
      if (i < 2) return false;    // недостаточно баров для сравнения — входы разрешены
      const prevMa = cfg.eqCalcMAArr[i-2];
      return prevMa > 0 && ma < prevMa;  // block ТОЛЬКО если MA была прогрета И падает
    },
    blocksS:  (cfg, i) => {
      if (!cfg.eqCalcMAArr) return false;
      const ma = cfg.eqCalcMAArr[i-1];
