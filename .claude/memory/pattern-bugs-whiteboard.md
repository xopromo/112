...
## ПАТТЕРН #5: 70% Data Truncation Pattern

**STATUS: OPEN** 🔴  
**Last Updated:** 2026-04-05  
**Total Cases:** 3+ (найдено, возможно больше)  
**Confidence:** Требуется FULL SEARCH  
**Last Wave:** 1 (исправлены 3 места, но баг ВСЕ ЕЩЁ ВИДИМ!)  
**Regression Status:** ❌ БАГ ЕЩЁ СУЩЕСТВУЕТ (графики показывают 7000 вместо 10000)

**Определение:**
Когда сохраняются OOS результаты, данные обрезаются на 70% (IS часть) вместо полных 100%.
Это вызывает потерю данных при рендеринге графиков и несинхронизированное отображение линий.

**Правило:** OOS Full Data Preservation  
**Уровень:** CRITICAL  
**Статус:** 🔴 НЕВЕРИФИЦИРОВАНО (исправления НЕ помогли! Баг остался)

### Cases этого паттерна:

#### Case 5.1: old_eq (ui_oos.js:1542)
```
ФАЙЛ: ui_oos.js:1542-1548
БЫЛО: Array.from(rOld.eq.slice(0, 0.70*length))
FIXED: Array.from(rOld.eq)
СТАТУС: Исправлено, но БАГ ОСТАЛСЯ! ❌
МАРКЕР: ##OOS_FULL_DATA_FIX##
```

#### Case 5.2: old_eqCalc (ui_oos.js:1570)
```
ФАЙЛ: ui_oos.js:1570
БЫЛО: Array.from(rOld.eqCalc.slice(0, 0.70*length))
FIXED: Array.from(rOld.eqCalc)
СТАТУС: Исправлено, но БАГ ОСТАЛСЯ! ❌
МАРКЕР: ##OOS_FULL_DATA_FIX##
```

#### Case 5.3: old_eqCalcBaselineArr (ui_oos.js:1615)
```
ФАЙЛ: ui_oos.js:1615
БЫЛО: rOld.eqCalcBaselineArr.slice(0, 0.70*length)
FIXED: Array.from(rOld.eqCalcBaselineArr)
СТАТУС: Исправлено, но БАГ ОСТАЛСЯ! ❌
МАРКЕР: ##OOS_FULL_DATA_FIX##
```
...
**⚠️ КРИТИЧНО:** Исправления сделаны, но графики ВСЕ ЕЩЁ показывают 7000 баров!
Это означает что:
1. Либо исправления в неправильных местах
2. Либо 7000 выводится из другого источника (localStorage, кэш, другой код)
3. Либо нужна FULL SEARCH для поиска всех мест где происходит обрезка

**Дальнейшие действия:**
1. Запустить FULL SEARCH для всех `.slice(0, 0.70*)` в кодебазе
2. Проверить где выводится информация "7000" (может быть это другой источник)
3. Убедиться что исправления ДЕЙСТВИТЕЛЬНО применены (пересобран bundle?)
4. Включить диагностику и проверить действительные значения
