...
# ======================================================================
# RULE 20: Защита от 70% Data Truncation Pattern (ПАТТЕРН #5)
# ======================================================================
# ПАТТЕРН #5: Везде где OOS результаты, данные могут быть обрезаны на 70%
# Правило: 100% данные сохраняются в результаты, 70% используется ТОЛЬКО для метрик
# ======================================================================
echo ""
echo "RULE 20: 70% Data Truncation (ПАТТЕРН #5)"

if [ -f ui_oos.js ]; then
  # Ищем места где может быть обрезка (0.70 * .length) в полях данных
  TRUNCATION_FOUND=$(grep -n "\.slice(0.*0\.70\|\.slice(0.*Math.round.*0\.70" ui_oos.js | grep -v "##OOS_FULL_DATA_FIX##" | wc -l)

  if [ "$TRUNCATION_FOUND" -gt 0 ]; then
    echo "  ⚠️  НАЙДЕНА обрезка 70% без защиты (##OOS_FULL_DATA_FIX##):"
    grep -n "\.slice(0.*0\.70\|\.slice(0.*Math.round.*0\.70" ui_oos.js | grep -v "##OOS_FULL_DATA_FIX##" | head -5
    echo ""
    echo "  ПРАВИЛО: Эти поля должны содержать ПОЛНЫЕ данные (100%), не обрезанные!"
    echo "  Если это IS данные для метрик - окей, но они НЕ должны быть в OOS результатах!"
    ERRORS=$((ERRORS + 1))
  else
    echo "  ✓ Нет необнаруженной обрезки 70% (все защищены ##OOS_FULL_DATA_FIX##)"
  fi
else
  echo "  ⚠️  ui_oos.js не найден - пропускаем RULE 20"
fi
