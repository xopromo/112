...
// ── TV пивоты из CSV (если экспортированы через test/pivot_debug.pine) ──────
// Для получения: добавить pivot_debug.pine как индикатор в TV, экспортировать
// CSV с колонками PV_HI / PV_LO, добавить их в tv_equity.csv или передать
// отдельным файлом (--pvcsv путь) → раздел ниже покажет полное сравнение.
const tvPvHi = new Uint8Array(N);
const tvPvLo = new Uint8Array(N);
let hasTvPivots = false;
const pvCsvArg = process.argv.indexOf('--pvcsv');
const pvCsvPath = pvCsvArg >= 0 ? process.argv[pvCsvArg + 1] : null;
const pvRows = pvCsvPath ? parseCSV(pvCsvPath) : null;
if (pvRows) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    if (r) {
      const colHi = Object.keys(r).find(k => k.includes('pv_hi') || k.includes('pvhi'));
      const colLo = Object.keys(r).find(k => k.includes('pv_lo') || k.includes('pvlo'));
      if (colHi) { tvPvHi[i] = parseFloat(r[colHi]) > 0 ? 1 : 0; hasTvPivots = true; }
      if (colLo) { tvPvLo[i] = parseFloat(r[colLo]) > 0 ? 1 : 0; }
    }
  }
} else {
  // Попробовать найти PV_HI/PV_LO в основном tv_equity.csv
  for (let i = 0; i < N; i++) {
    const tv = tvMap[DATA[i].t];
    if (tv && (tv.pv_hi !== undefined && tv.pv_hi !== '')) {
...
// Float32: Pine может округлять high до float32 перед сравнением
function calcPivotHighF32(left, right) {
  const N = DATA.length, res = new Uint8Array(N);
  for (let i = left + right; i < N; i++) {
    const idx = i - right, v = Math.fround(DATA[idx].h);
    let ok = true;
    for (let j = idx - left; j < idx; j++) { if (Math.fround(DATA[j].h) > v) { ok = false; break; } }
    if (ok) for (let j = idx + 1; j <= Math.min(idx + right, N-1); j++) { if (Math.fround(DATA[j].h) >= v) { ok = false; break; } }
    if (ok) res[i] = 1;
  }
  return res;
}
const pvHiF32 = calcPivotHighF32(6, 5);
...
// ── Сравнение пивотов TV vs JS ────────────────────────────────────────────────
console.log('\n=== СРАВНЕНИЕ ПИВОТОВ TV vs JS ===');
if (!hasTvPivots) {
  console.log('⚠️  TV-пивоты не найдены в CSV.');
  console.log('   Чтобы получить TV-пивоты:');
  console.log('   1. Открыть test/pivot_debug.pine в TradingView как отдельный индикатор');
  console.log('   2. Настроить pvL=6, pvR=5 (или другие параметры стратегии)');
  console.log('   3. Экспортировать данные графика: Export chart data → CSV');
  console.log('   4. Запустить: node test/tv_diag.js --pvcsv /path/to/exported.csv');
  console.log('');
  console.log('   Альтернатива: добавить колонки PV_HI/PV_LO прямо в tv_equity.csv');
} else {
  const pvL_p = 6, pvR_p = 5;

  // Статистика по всем 4 вариантам JS
  function pivotStats(jsArr, tvArr, label) {
    let both=0, jsOnly=0, tvOnly=0;
    for (let i=0; i<N; i++) {
      if (jsArr[i] && tvArr[i]) both++;
      else if (jsArr[i] && !tvArr[i]) jsOnly++;
      else if (!jsArr[i] && tvArr[i]) tvOnly++;
    }
    const tvTotal = both + tvOnly;
    const jsTotal = both + jsOnly;
    const prec = tvTotal>0 ? (both/jsTotal*100).toFixed(1) : '-';
    const rec  = tvTotal>0 ? (both/tvTotal*100).toFixed(1) : '-';
    console.log(`  ${label.padEnd(35)} TV=${tvTotal} JS=${jsTotal} both=${both} JS-only=${jsOnly} TV-only=${tvOnly}  precision=${prec}% recall=${rec}%`);
    return { both, jsOnly, tvOnly, jsTotal, tvTotal };
  }

  console.log('pvHi (pivot high, left=6, right=5):');
  pivotStats(pvHi,           tvPvHi, 'current (left non-strict, right strict)');
  pivotStats(pvHiStrict,     tvPvHi, 'strict  (both sides strict >=/>)');
  pivotStats(pvHiLeftStrict, tvPvHi, 'leftStrict (left >=, right >)');
  pivotStats(pvHiF32,        tvPvHi, 'float32 (Math.fround before compare)');

  console.log('\npvLo (pivot low, left=6, right=5):');
  function calcPivotLowF32(left, right) {
    const N = DATA.length, res = new Uint8Array(N);
    for (let i = left + right; i < N; i++) {
      const idx = i - right, v = Math.fround(DATA[idx].l);
      let ok = true;
      for (let j = idx - left; j < idx; j++) { if (Math.fround(DATA[j].l) < v) { ok = false; break; } }
      if (ok) for (let j = idx + 1; j <= Math.min(idx + right, N-1); j++) { if (Math.fround(DATA[j].l) <= v) { ok = false; break; } }
      if (ok) res[i] = 1;
    }
    return res;
  }
  const pvLoF32 = calcPivotLowF32(6, 5);
  pivotStats(pvLo,    tvPvLo, 'current');
  pivotStats(pvLoF32, tvPvLo, 'float32');

  // Найти JS-only пивоты (JS видит, TV нет) — корень проблемы
  console.log('\n=== JS-ONLY pvHi (ложные пивоты — TV не видит) ===');
  console.log('Бар#  | center | v_center   | ближ_осед  | Δ          | сосед_бар  | подозрение');
  let jsOnlyCount = 0;
  for (let i = 0; i < N; i++) {
    if (!pvHi[i] || tvPvHi[i]) continue; // только JS-only
    const center = i - pvR_p;
    if (center < 0 || center >= N) continue;
    const v = DATA[center].h;

    // Найти ближайший бар-сосед с максимальным high (который TV мог бы использовать для дисквалификации)
    let maxNeighH = 0, maxNeighBar = -1;
    for (let j = center - pvL_p; j <= center + pvR_p; j++) {
      if (j === center || j < 0 || j >= N) continue;
      if (DATA[j].h > maxNeighH) { maxNeighH = DATA[j].h; maxNeighBar = j; }
    }
    const delta = v - maxNeighH;
    const f32v = Math.fround(v), f32n = Math.fround(maxNeighH);
    const suspicion = f32v === f32n ? '⚠️ float32 равны!' :
                      delta < 1e-5  ? `⚠️ Δ<1e-5 (${delta.toExponential(2)})` :
                      delta < 1e-4  ? `Δ<1e-4 (${delta.toExponential(2)})` : '';
    console.log(
      `  #${String(i).padStart(5)} | #${String(center).padStart(5)} | ${v.toFixed(7).padStart(10)} | ${maxNeighH.toFixed(7).padStart(10)} | ${delta.toExponential(2).padStart(10)} | #${String(maxNeighBar).padStart(5)} | ${suspicion}`
    );
    if (++jsOnlyCount >= 30) { console.log('  ... (показано 30)'); break; }
  }
  if (jsOnlyCount === 0) console.log('  Нет! JS и TV видят одинаковые pvHi ✅');

  // TV-only пивоты (TV видит, JS нет) — JS пропускает
  console.log('\n=== TV-ONLY pvHi (JS пропускает — проблема recall) ===');
  let tvOnlyCount = 0;
  for (let i = 0; i < N; i++) {
    if (tvPvHi[i] && !pvHi[i]) {
      const center = i - pvR_p;
      const v = center >= 0 ? DATA[center].h : 0;
      console.log(`  #${String(i).padStart(5)} | center=#${String(center).padStart(5)} | h=${v.toFixed(7)}`);
      if (++tvOnlyCount >= 20) { console.log('  ... (показано 20)'); break; }
    }
  }
  if (tvOnlyCount === 0) console.log('  Нет! JS видит все TV-пивоты ✅');

  // Лучший вариант по precision (меньше JS-only = меньше лишних сигналов)
  console.log('\n=== ВЫВОД: какой вариант JS лучше совпадает с TV? ===');
  const variants = [
    { name: 'current', arr: pvHi },
    { name: 'strict',  arr: pvHiStrict },
    { name: 'leftStr', arr: pvHiLeftStrict },
    { name: 'float32', arr: pvHiF32 },
  ];
  for (const v of variants) {
    let both=0, jsOnly=0, tvOnly=0;
    for (let i=0; i<N; i++) {
      if (v.arr[i] && tvPvHi[i]) both++;
      else if (v.arr[i]) jsOnly++;
      else if (tvPvHi[i]) tvOnly++;
    }
    const F1 = both>0 ? (2*both/(2*both+jsOnly+tvOnly)*100).toFixed(1) : '0';
    console.log(`  ${v.name.padEnd(10)} F1=${F1}%  jsOnly=${jsOnly}  tvOnly=${tvOnly}`);
  }
}
