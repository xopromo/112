...
async function runOpt() {
  ...
  function _attachOOS(cfg) {
    if (!_useOOS) return;
    // ПОЧЕМУ fullDATA, а не отдельный бэктест на последних 30%:
    //   Если запускать бэктест только на OOS-срезе, индикаторы (MA, ATR, ADX)
    //   прогреваются заново с нуля — первые N баров OOS дают неверные сигналы.
    //   Решение: один бэктест на всех данных → непрерывная equity-кривая → делим
    //   кривую по _isN. Прогрев к моменту split уже завершён.
    //
    // ПОЧЕМУ retention, а не raw OOS PnL:
    //   Raw PnL зависит от длины OOS-периода и масштаба стратегии.
    //   Стратегия +80% IS / +5% OOS выглядит "нормально" по raw PnL, но
    //   фактически деградировала в 16× — это явная подгонка под IS.
    //   Retention = (OOS PnL / OOS баров) / (IS PnL / IS баров) — единица
    //   измерения "скорость роста сохранена". 1.0 = OOS растёт как IS. 0.3 = порог.
    //
    // ПОЧЕМУ порог minIsGain (40% от totalGain):
    //   Если IS почти не вырос (isGain ≈ 0), деление oosRate/isRate → ∞.
    //   Типичный кейс: просадка в IS восстанавливается к split → isGain≈0.
    //   Защита: если IS не внёс ≥40% суммарного прироста → retention = -1.
    //
