...
async function runOOSOnNewData() {
  ...
  for (let i = 0; i < srcList.length; i++) {
    ...
    // ##EQ_MA_FILTER## Рассчитаем индекс раздела IS/OOS для правильного split eq_old
    const _isDataLen = origDATA.length;
    const _isN = Math.round(_isDataLen * 0.70);  // 70% точка в исходных данных
    ...
    _oosTableResults.push({
      ...
      // ##EQ_MA_FILTER## КРИТИЧНО: рассчитываем точный split для eq_old (только IS часть, не полная)
      // rOld.eq = полная кривая (100% данных), но нужно вернуть только IS часть (70%)
      //映射: eq_split_idx ≈ (0.70 * eq_length), так как рыночные данные распределены пропорционально
      old_eq:    (() => {
