...
| `buildName` | 155 | Строит имя результата |
| `runOpt` | 286 | Основная оптимизация (MC/TPE/exhaustive) |
| `_runOOS` | 327 | Запуск бэктеста на срезе данных (внутри runOpt) |
| `_attachOOS` | 342 | Вычисляет IS/OOS split и прикрепляет к cfg._oos |
| `_calcIndicators` | 1740 | Вычисляет индикаторы по DATA |
| `buildBtCfg` | 1915 | Строит конфиг бэктеста из cfg+ind |
| `runMassRobust` | 2062 | Массовый тест устойчивости |
| `runRobustScoreFor` | 2128 | Тест устойчивости для одного результата |
| `runRobustScoreForDetailed` | 2295 | То же, но с деталями по каждому тесту |
| `HC_NUMERIC_PARAMS` | 2361 | Единый список числовых параметров HC |
...
| `runOOSScan` | 2859 | OOS-скан видимых результатов |
| `openHCModal` | 3046 | Открыть модал поиска соседей |
| `_hcRobScore` | 3145 | Робастность для GA-поиска |
| `_hcRunBacktest` | 3349 | Быстрый бэктест для HC (с кэшем `_robSliceCache`) |
| `_hcNeighbours` | 3529 | Генерация соседних cfg (содержит `mutate()`) |
| `runHillClimbing` | 3660 | Главный HC алгоритм |
| `_hcOpenDetail` | 4416 | Открыть детальный вид из HC модала |
...
| `_addActivePinev6` | 1109 | Добавляет `active=` для toggle-групп |
