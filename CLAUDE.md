...
| `_calcIndicators` | 1966 | Вычисляет индикаторы по DATA |
| `buildBtCfg` | 2260 | Строит конфиг бэктеста из cfg+ind |
| `runMassRobust` | 2438 | Массовый тест устойчивости |
| `runRobustScoreFor` | 2504 | Тест устойчивости для одного результата |
| `runRobustScoreForDetailed` | 2671 | То же, но с деталями по каждому тесту |
| `HC_NUMERIC_PARAMS` | 2737 | Единый список числовых параметров HC |
...
| `runOOSScan` | 2934 | OOS-скан видимых результатов |
| `openHCModal` | 3125 | Открыть модал поиска соседей |
| `_hcRobScore` | 3224 | Робастность для GA-поиска |
| `_hcRunBacktest` | 3428 | Быстрый бэктест для HC (с кэшем `_robSliceCache`) |
| `_hcNeighbours` | 3612 | Генерация соседних cfg (содержит `mutate()`) |
| `runHillClimbing` | 3743 | Главный HC алгоритм |
| `_hcOpenDetail` | 4502 | Открыть детальный вид из HC модала |
