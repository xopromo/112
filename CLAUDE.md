...
git add -A && git commit -m "..."      # коммит
git push -u origin <claude/ветка>      # пуш
```
...
| `resetAllFilters` | 544 | Сброс всех фильтров |
| `applyFilters` | 559 | Применение фильтров к таблице |
| `runOOSScan` | 3121 | OOS-скан видимых результатов |
| `openHCModal` | 3314 | Открыть модал поиска соседей |
| `_hcRobScore` | 3413 | Робастность для GA-поиска |
| `_hcRunBacktest` | 3617 | Быстрый бэктест для HC (с кэшем `_robSliceCache`) |
| `_hcNeighbours` | 3804 | Генерация соседних cfg (содержит `mutate()`) |
| `runHillClimbing` | 3935 | Главный HC алгоритм |
| `_hcOpenDetail` | 4696 | Открыть детальный вид из HC модала |
