...
| `runOpt` | 353 | Основная оптимизация (MC/TPE/exhaustive) |
| `_runOOS` | 394 | Запуск бэктеста на срезе данных (внутри runOpt) |
| `_attachOOS` | 409 | Вычисляет IS/OOS split и прикрепляет к cfg._oos |
| `_calcIndicators` | 1836 | Вычисляет индикаторы по DATA |
| `buildBtCfg` | 2101 | Строит конфиг бэктеста из cfg+ind |
| `runMassRobust` | 2250 | Массовый тест устойчивости |
| `runRobustScoreFor` | 2316 | Тест устойчивости для одного результата |
| `runRobustScoreForDetailed` | 2483 | То же, но с деталями по каждому тесту |
| `HC_NUMERIC_PARAMS` | 2549 | Единый список числовых параметров HC |
