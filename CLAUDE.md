...
| `buildName` | 236 | Строит имя результата |
| `runOpt` | 358 | Основная оптимизация (MC/TPE/exhaustive) |
| `_runOOS` | 399 | Запуск бэктеста на срезе данных (внутри runOpt) |
| `_attachOOS` | 414 | Вычисляет IS/OOS split и прикрепляет к cfg._oos |
| `_calcIndicators` | 1867 | Вычисляет индикаторы по DATA |
| `buildBtCfg` | 2161 | Строит конфиг бэктеста из cfg+ind |
| `runMassRobust` | 2339 | Массовый тест устойчивости |
| `runRobustScoreFor` | 2405 | Тест устойчивости для одного результата |
| `runRobustScoreForDetailed` | 2572 | То же, но с деталями по каждому тесту |
| `HC_NUMERIC_PARAMS` | 2638 | Единый список числовых параметров HC |

**ui.js** (интерфейс + HC):
| Функция | Строка | Описание |
|---------|--------|----------|
| `switchTableMode` | 362 | Переключение HC/Fav/Results режимов |
| `resetAllFilters` | 420 | Сброс всех фильтров |
| `applyFilters` | 435 | Применение фильтров к таблице |
| `runOOSScan` | 2925 | OOS-скан видимых результатов |
| `openHCModal` | 3116 | Открыть модал поиска соседей |
| `_hcRobScore` | 3215 | Робастность для GA-поиска |
| `_hcRunBacktest` | 3419 | Быстрый бэктест для HC (с кэшем `_robSliceCache`) |
| `_hcNeighbours` | 3603 | Генерация соседних cfg (содержит `mutate()`) |
| `runHillClimbing` | 3734 | Главный HC алгоритм |
| `_hcOpenDetail` | 4493 | Открыть детальный вид из HC модала |

**pine_export.js** (экспорт Pine Script):
| Функция | Строка | Описание |
|---------|--------|----------|
| `generatePineScript` | 14 | Главная функция экспорта в Pine v6 |
| `fixPineScript` | 1128 | Автоисправление ошибок Pine |
| `_addActivePinev6` | 1244 | Добавляет `active=` для toggle-групп |

**core.js** (движок бэктеста):
| Функция | Строка | Описание |
|---------|--------|----------|
| `backtest` | 282 | Основной бэктест-цикл |
