...
> **ПЕРВОЕ ДЕЙСТВИЕ В КАЖДОЙ СЕССИИ:** запусти проверку инструментов:
> ```bash
> bash agent/check_context_mode.sh
> ```
> Если выводит "OK" — продолжай. Если нет — скрипт автоматически переустановит.
...
4. Если агент не работает — запусти `bash agent/setup.sh`
...
| Файл | Роль |
|------|------|
| `agent/setup_context_mode.sh` | Полная установка с нуля |
| `agent/check_context_mode.sh` | Быстрая проверка + автопереустановка |
...
**Статистика экономии за сессию** (запускай в конце сессии чтобы увидеть результат):
```
/context-mode:stats
```
Показывает: сколько KB было сжато, коэффициент сжатия по каждому инструменту (Read, Grep, Bash и т.д.), итоговую экономию токенов за сессию.
