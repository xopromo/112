...
| `runOpt` | 364 | Основная оптимизация (MC/TPE/exhaustive) |
| `_runOOS` | 405 | Запуск бэктеста на срезе данных (внутри runOpt) |
| `_attachOOS` | 420 | Вычисляет IS/OOS split и прикрепляет к cfg._oos |
| `_calcIndicators` | 1800 | Вычисляет индикаторы по DATA |
| `buildBtCfg` | 2065 | Строит конфиг бэктеста из cfg+ind |
| `runMassRobust` | 2214 | Массовый тест устойчивости |
| `runRobustScoreFor` | 2280 | Тест устойчивости для одного результата |
| `runRobustScoreForDetailed` | 2447 | То же, но с деталями по каждому тесту |
| `HC_NUMERIC_PARAMS` | 2513 | Единый список числовых параметров HC |
