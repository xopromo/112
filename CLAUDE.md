...
| `runOpt` | 357 | Основная оптимизация (MC/TPE/exhaustive) |
| `_runOOS` | 398 | Запуск бэктеста на срезе данных (внутри runOpt) |
| `_attachOOS` | 413 | Вычисляет IS/OOS split и прикрепляет к cfg._oos |
| `_calcIndicators` | 1859 | Вычисляет индикаторы по DATA |
| `buildBtCfg` | 2124 | Строит конфиг бэктеста из cfg+ind |
| `runMassRobust` | 2273 | Массовый тест устойчивости |
| `runRobustScoreFor` | 2339 | Тест устойчивости для одного результата |
| `runRobustScoreForDetailed` | 2506 | То же, но с деталями по каждому тесту |
| `HC_NUMERIC_PARAMS` | 2572 | Единый список числовых параметров HC |
...
| `runOOSScan` | 2922 | OOS-скан видимых результатов |
| `openHCModal` | 3113 | Открыть модал поиска соседей |
| `_hcRobScore` | 3212 | Робастность для GA-поиска |
| `_hcRunBacktest` | 3416 | Быстрый бэктест для HC (с кэшем `_robSliceCache`) |
| `_hcNeighbours` | 3600 | Генерация соседних cfg (содержит `mutate()`) |
| `runHillClimbing` | 3731 | Главный HC алгоритм |
| `_hcOpenDetail` | 4490 | Открыть детальный вид из HC модала |

**pine_export.js** (экспорт Pine Script):
| Функция | Строка | Описание |
|---------|--------|----------|
| `generatePineScript` | 14 | Главная функция экспорта в Pine v6 |
| `fixPineScript` | 1102 | Автоисправление ошибок Pine |
| `_addActivePinev6` | 1218 | Добавляет `active=` для toggle-групп |

**core.js** (движок бэктеста):
| Функция | Строка | Описание |
|---------|--------|----------|
| `backtest` | 256 | Основной бэктест-цикл |
