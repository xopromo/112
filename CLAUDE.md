...
| `_calcIndicators` | 1943 | Вычисляет индикаторы по DATA |
| `buildBtCfg` | 2237 | Строит конфиг бэктеста из cfg+ind |
| `runMassRobust` | 2415 | Массовый тест устойчивости |
| `runRobustScoreFor` | 2481 | Тест устойчивости для одного результата |
| `runRobustScoreForDetailed` | 2648 | То же, но с деталями по каждому тесту |
| `HC_NUMERIC_PARAMS` | 2714 | Единый список числовых параметров HC |
...
| `runOOSScan` | 2933 | OOS-скан видимых результатов |
| `openHCModal` | 3124 | Открыть модал поиска соседей |
| `_hcRobScore` | 3223 | Робастность для GA-поиска |
| `_hcRunBacktest` | 3427 | Быстрый бэктест для HC (с кэшем `_robSliceCache`) |
| `_hcNeighbours` | 3611 | Генерация соседних cfg (содержит `mutate()`) |
| `runHillClimbing` | 3742 | Главный HC алгоритм |
| `_hcOpenDetail` | 4501 | Открыть детальный вид из HC модала |
