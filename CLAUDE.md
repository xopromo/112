...
| `runOpt` | 450 | Основная оптимизация (MC/TPE/exhaustive) |
| `_runOOS` | 491 | Запуск бэктеста на срезе данных (внутри runOpt) |
| `_attachOOS` | 506 | Вычисляет IS/OOS split и прикрепляет к cfg._oos |
| `_calcIndicators` | 2118 | Вычисляет индикаторы по DATA |
| `buildBtCfg` | 2416 | Строит конфиг бэктеста из cfg+ind |
| `runMassRobust` | 2603 | Массовый тест устойчивости |
| `runRobustScoreFor` | 2669 | Тест устойчивости для одного результата |
| `runRobustScoreForDetailed` | 2836 | То же, но с деталями по каждому тесту |
| `HC_NUMERIC_PARAMS` | 2902 | Единый список числовых параметров HC |
...
| `runOOSScan` | 2974 | OOS-скан видимых результатов |
| `openHCModal` | 3167 | Открыть модал поиска соседей |
| `_hcRobScore` | 3266 | Робастность для GA-поиска |
| `_hcRunBacktest` | 3470 | Быстрый бэктест для HC (с кэшем `_robSliceCache`) |
| `_hcNeighbours` | 3657 | Генерация соседних cfg (содержит `mutate()`) |
| `runHillClimbing` | 3788 | Главный HC алгоритм |
| `_hcOpenDetail` | 4549 | Открыть детальный вид из HC модала |

**pine_export.js** (экспорт Pine Script):
| Функция | Строка | Описание |
|---------|--------|----------|
| `generatePineScript` | 14 | Главная функция экспорта в Pine v6 |
| `fixPineScript` | 1219 | Автоисправление ошибок Pine |
| `_addActivePinev6` | 1335 | Добавляет `active=` для toggle-групп |

**core.js** (движок бэктеста):
| Функция | Строка | Описание |
|---------|--------|----------|
| `backtest` | 308 | Основной бэктест-цикл |
