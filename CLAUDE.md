...
| `runOpt` | 442 | Основная оптимизация (MC/TPE/exhaustive) |
| `_runOOS` | 483 | Запуск бэктеста на срезе данных (внутри runOpt) |
| `_attachOOS` | 498 | Вычисляет IS/OOS split и прикрепляет к cfg._oos |
| `_calcIndicators` | 2056 | Вычисляет индикаторы по DATA |
| `buildBtCfg` | 2348 | Строит конфиг бэктеста из cfg+ind |
| `runMassRobust` | 2526 | Массовый тест устойчивости |
| `runRobustScoreFor` | 2592 | Тест устойчивости для одного результата |
| `runRobustScoreForDetailed` | 2759 | То же, но с деталями по каждому тесту |
| `HC_NUMERIC_PARAMS` | 2825 | Единый список числовых параметров HC |
