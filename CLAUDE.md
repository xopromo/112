...
| `buildName` | 222 | Строит имя результата |
| `runOpt` | 344 | Основная оптимизация (MC/TPE/exhaustive) |
| `_runOOS` | 385 | Запуск бэктеста на срезе данных (внутри runOpt) |
| `_attachOOS` | 400 | Вычисляет IS/OOS split и прикрепляет к cfg._oos |
| `_calcIndicators` | 1850 | Вычисляет индикаторы по DATA |
| `buildBtCfg` | 2144 | Строит конфиг бэктеста из cfg+ind |
| `runMassRobust` | 2322 | Массовый тест устойчивости |
| `runRobustScoreFor` | 2388 | Тест устойчивости для одного результата |
| `runRobustScoreForDetailed` | 2555 | То же, но с деталями по каждому тесту |
| `HC_NUMERIC_PARAMS` | 2621 | Единый список числовых параметров HC |
...
| `fixPineScript` | 1074 | Автоисправление ошибок Pine |
| `_addActivePinev6` | 1190 | Добавляет `active=` для toggle-групп |
