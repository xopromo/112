...
| `_runOOS` | 266 | Запуск бэктеста на срезе данных (внутри runOpt) |
| `_attachOOS` | 281 | Вычисляет IS/OOS split и прикрепляет к cfg._oos |
| `_calcIndicators` | 1679 | Вычисляет индикаторы по DATA |
| `buildBtCfg` | 1854 | Строит конфиг бэктеста из cfg+ind |
| `runMassRobust` | 2001 | Массовый тест устойчивости |
| `runRobustScoreFor` | 2067 | Тест устойчивости для одного результата |
| `runRobustScoreForDetailed` | 2234 | То же, но с деталями по каждому тесту |
| `HC_NUMERIC_PARAMS` | 2300 | Единый список числовых параметров HC |
...
| `_hcRunBacktest` | 3325 | Быстрый бэктест для HC (с кэшем `_robSliceCache`) |
| `_hcNeighbours` | 3505 | Генерация соседних cfg (содержит `mutate()`) |
| `runHillClimbing` | 3636 | Главный HC алгоритм |
| `_hcOpenDetail` | 4391 | Открыть детальный вид из HC модала |
