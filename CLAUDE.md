...
| `buildName` | 114 | Строит имя результата |
| `runOpt` | 245 | Основная оптимизация (MC/TPE/exhaustive) |
| `_runOOS` | 286 | Запуск бэктеста на срезе данных (внутри runOpt) |
| `_attachOOS` | 301 | Вычисляет IS/OOS split и прикрепляет к cfg._oos |
| `_calcIndicators` | 1699 | Вычисляет индикаторы по DATA |
| `buildBtCfg` | 1874 | Строит конфиг бэктеста из cfg+ind |
| `runMassRobust` | 2021 | Массовый тест устойчивости |
| `runRobustScoreFor` | 2087 | Тест устойчивости для одного результата |
| `runRobustScoreForDetailed` | 2254 | То же, но с деталями по каждому тесту |
| `HC_NUMERIC_PARAMS` | 2320 | Единый список числовых параметров HC |
...
| `runOOSScan` | 2838 | OOS-скан видимых результатов |
| `openHCModal` | 3025 | Открыть модал поиска соседей |
| `_hcRobScore` | 3124 | Робастность для GA-поиска |
| `_hcRunBacktest` | 3328 | Быстрый бэктест для HC (с кэшем `_robSliceCache`) |
| `_hcNeighbours` | 3508 | Генерация соседних cfg (содержит `mutate()`) |
| `runHillClimbing` | 3639 | Главный HC алгоритм |
| `_hcOpenDetail` | 4395 | Открыть детальный вид из HC модала |
