...
| `_calcIndicators` | 2186 | Вычисляет индикаторы по DATA |
| `buildBtCfg` | 2489 | Строит конфиг бэктеста из cfg+ind |
| `runMassRobust` | 2685 | Массовый тест устойчивости |
| `runRobustScoreFor` | 2751 | Тест устойчивости для одного результата |
| `runRobustScoreForDetailed` | 2918 | То же, но с деталями по каждому тесту |
| `HC_NUMERIC_PARAMS` | 2984 | Единый список числовых параметров HC |
...
| `runOOSScan` | 2976 | OOS-скан видимых результатов |
| `openHCModal` | 3169 | Открыть модал поиска соседей |
| `_hcRobScore` | 3268 | Робастность для GA-поиска |
| `_hcRunBacktest` | 3472 | Быстрый бэктест для HC (с кэшем `_robSliceCache`) |
| `_hcNeighbours` | 3659 | Генерация соседних cfg (содержит `mutate()`) |
| `runHillClimbing` | 3790 | Главный HC алгоритм |
| `_hcOpenDetail` | 4551 | Открыть детальный вид из HC модала |

**pine_export.js** (экспорт Pine Script):
| Функция | Строка | Описание |
|---------|--------|----------|
| `generatePineScript` | 14 | Главная функция экспорта в Pine v6 |
| `fixPineScript` | 1248 | Автоисправление ошибок Pine |
| `_addActivePinev6` | 1364 | Добавляет `active=` для toggle-групп |
