...
```bash
# 1. Обновить main и разрешить конфликты
...
- `auto-resolve-conflicts.sh` разрешает конфликты в бандлах (`.html`, `.js`)
- Для исходного кода требует ручного вмешательства
- Автоматически пересобирает бандл после разрешения
...
| **auto-resolve-conflicts.sh** | Авторазрешение конфликтов в бандлах | `bash .claude/scripts/auto-resolve-conflicts.sh` |
