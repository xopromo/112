...
function openOOSDiagnostic() {
  ...
  text += `📍 ИСХОДНЫЙ ПЕРИОД (История · IS):\n`;
  ...
  text += `📍 НОВЫЙ ПЕРИОД (Загруженные данные · OOS):\n`;
  ...
  if (oosR.old_pnl != null && oosR.new_pnl != null) {
    const dPnl = oosR.new_pnl - oosR.old_pnl;
    if (dPnl < -50) {
      text += `   ❌ КРИТИЧЕСКОЕ ПАДЕНИЕ PnL на ${Math.abs(dPnl).toFixed(1)}%\n`;
      hasIssues = true;
    } else if (dPnl < -10) {
      text += `   ⚠️  Значительное снижение PnL на ${Math.abs(dPnl).toFixed(1)}%\n`;
    } else if (dPnl > 20) {
  ...
  if (oosR.old_wr != null && oosR.new_wr != null) {
    const dWr = oosR.new_wr - oosR.old_wr;
    if (Math.abs(dWr) > 20) {
      text += `   ⚠️  Резкое изменение WR на ${dWr > 0 ? '+' : ''}${dWr.toFixed(1)}% (переоптимизация?)\n`;
    }
  }
  ...
  if (oosR.old_n != null && oosR.new_n != null) {
    ...
    if (ratio < 0.5) {
      text += `   ⚠️  Кол-во сделок упало на ${Math.round((1 - ratio) * 100)}% (нет условий на новых данных?)\n`;
    }
  }
  ...
  if (oosR.old_dd != null && oosR.new_dd != null) {
    const dDd = oosR.new_dd - oosR.old_dd;
    if (dDd > 20) {
      text += `   ❌ DD критически вырос на ${dDd.toFixed(1)}% (управление риском?)\n`;
      hasIssues = true;
    }
  }

  if (!hasIssues && oosR.new_pnl > 0) {
    text += `   ✅ Результаты стабильны между периодами\n`;
  }
