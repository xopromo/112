...
switchTableMode = function(mode) {
  ...
  if (mode === 'wfo') {
    ...
    if (stdScroll) stdScroll.style.display = 'none';
    if (oosTbl)    oosTbl.style.display    = 'none';
    if (wfoWrap)   wfoWrap.style.display   = 'block';
...
let _wfoVisibleResults = []; // отфильтрованные/отсортированные строки (для клика)
...
function renderWfoTable() {
  ...
  _wfoVisibleResults = filtered; // сохраняем для обработчиков кликов
  ...
  for (let i = 0; i < filtered.length; i++) {
    ...
    const winTip = (r.windows || []).map((w, j) => {
      ...
      return `Окно ${j+1}: ${is} | ${oos}`;
    }).join('&#10;');
    html += `<tr data-i="${i}" style="cursor:pointer" title="Клик — equity-график | Двойной клик — детали">
  ...
  tbody.onclick = function(e) {
    ...
    if (!tr) return;
    const idx = +tr.dataset.i;
    const r = _wfoVisibleResults[idx];
    if (!r) return;
    // Снимаем предыдущее выделение в WFO и основной таблице
    document.querySelectorAll('#wfo-tbody tr.sel').forEach(el => el.classList.remove('sel'));
    tr.classList.add('sel');
    // Показываем equity-график (та же канва, что в основной таблице)
    const eqc = document.getElementById('eqc');
    if (eqc) { eqc.style.display = ''; }
    drawEquityForResult(r);
  };
  tbody.ondblclick = function(e) {
    const tr = e.target.closest('tr[data-i]');
    if (!tr) return;
    const r = _wfoVisibleResults[+tr.dataset.i];
    if (!r) return;
    showDetail(r);
  };
