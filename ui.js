...
// ============================================================
// ui.js — ОГЛАВЛЕНИЕ  (ищи маркер ##UI_SECTION## grep'ом)
// ============================================================
// ##UI_DATA##     — ДАННЫЕ: loadFile, parseCSV, setLogic, updateVolStatus
// ##UI_TABLE##    — ТАБЛИЦА/ФИЛЬТРЫ: switchTableMode, applyFilters, renderVisibleResults
// ##UI_DETAIL##   — ДЕТАЛЬНАЯ ПАНЕЛЬ: showDetail, buildCopyText, closeDetail
// ##UI_OPT##      — ОПТИМИЗАТОР: setOptMode, setXMode, checkPause, yieldToUI, stopOpt, ETA
// ##UI_TPL##      — ШАБЛОНЫ + ИЗБРАННОЕ + СЕССИЯ: gatherSettings, applySettings, saveSession
// ##UI_ROBUST##   — ТЕСТ УСТОЙЧИВОСТИ: openRobustModal, runRobustTest, runOOSScan
// ##UI_PARSE##    — ПАРСЕР ТЕКСТА: parseTextToSettings, flashField, applyParsedText
// ##UI_EQUITY##   — ЭКВИТИ / СТАТИСТИКА: drawEquityData, drawEquity, doSort
// ##UI_HC##       — HILL CLIMBING + GA: openHCModal, runHillClimbing, _runGA, _hcRunBacktest
// ##UI_OOSCMP##   — OOS НА НОВЫХ ДАННЫХ: loadNewData, runOOSOnNewData, exportOOSTableCSV
// ============================================================
...
function renderVisibleResults() {
  ...
  for (let i = 0; i < page.length; i++) {
    ...
    // OOS⚡ ячейка: показываем retention (не raw PnL).
    // Пороги: ≥80% ✅ норма, 30–80% ⚠️ деградация, <30% ❌ переобучение.
    // Данные приходят из _attachOOS (opt.js) — см. там объяснение формулы.
...
// ============================================================
// ##UI_HEATMAP## — ТЕПЛОВАЯ КАРТА ПАРАМЕТРОВ
// Показывает устойчивость стратегии при изменении 2 параметров.
// ОТКАТ: удалить этот блок + кнопку btn-open-heatmap в shell.html + ##SECTION_HEATMAP##
// ============================================================

// Параметры доступные для осей; реально показываются только те, что есть в cfg стратегии
const _HM_PARAMS = [
...
function _hmRender(grid, xVals, yVals, xParam, yParam, metric, cX, cY) {
  ...
  for (let yi = 0; yi < grid.length; yi++) {
    ...
    for (let xi = 0; xi < (row ? row.length : 0); xi++) {
      ...
      const cy = PAD_T + (rows - 1 - yi) * cellH;  // y-axis: bottom = small, top = large
      ...
      // White border on current cfg value
