...
function runOOSTradeDiag(oosIdx, event) {
  ...
  const reader = new FileReader();
  reader.onload = e => {
    const tvTrades = _parseListOfTrades(e.target.result);
    if (!tvTrades || !tvTrades.length) {
      if (statusEl) statusEl.textContent = '❌ Не удалось прочитать CSV — ожидается List of Trades';
      return;
    }
    ...
    if (statusEl) statusEl.textContent =
      `✅ TV: ${tvTrades.length} сд. | Опт: ${optTrades.length} сд. | Расхождений: ${matchResult.issues.length} (${divCount} с парой)`;
