...
function _checkMSCConfig() {
  ...
  const meta = cfg.meta || {};
  const periodLabel = (meta.periodBStart && meta.periodBEnd)
    ? `период B: ${meta.periodBStart} → ${meta.periodBEnd}`
    : 'период B';

  // Build apply rows HTML
  const applyEntries = Object.entries(cfg.apply);
  const applyRows = applyEntries.map(([id, val]) => {
    const displayVal = typeof val === 'boolean' ? (val ? '✅ вкл' : '❌ выкл') : val;
    return `<span style="display:inline-flex;align-items:center;gap:6px;background:rgba(255,255,255,.06);border-radius:6px;padding:3px 8px;font-size:12px">
      <code style="color:var(--accent)">${id}</code>
      <span style="color:var(--fg2)">→</span>
      <b style="color:var(--fg)">${displayVal}</b>
    </span>`;
  }).join(' ');

  const hintsHtml = (cfg.hints || []).map(h =>
    `<div style="font-size:12px;color:var(--fg2);line-height:1.4">${h.icon || '•'} ${h.text}</div>`
  ).join('');
  ...
  banner.style.cssText = `
    ...
    box-shadow:0 4px 20px rgba(0,0,0,.5);max-width:680px;width:calc(100vw - 32px);
    display:flex;flex-direction:column;gap:10px;
  `;
  banner.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px">
      <div style="font-size:13px;font-weight:600;color:var(--accent)">
        📊 Конфигурация от Comparator
        <span style="font-weight:400;color:var(--fg2);margin-left:6px;font-size:12px">${periodLabel}</span>
      </div>
      <button onclick="document.getElementById('msc-banner').remove()" style="
        background:none;border:none;color:var(--fg2);cursor:pointer;font-size:16px;line-height:1;padding:0;flex-shrink:0
      ">✕</button>
    </div>
    <div style="display:flex;flex-wrap:wrap;gap:6px">${applyRows}</div>
    ${hintsHtml ? `<div id="msc-hints" style="display:none;border-top:1px solid rgba(255,255,255,.08);padding-top:8px;display:flex;flex-direction:column;gap:4px">${hintsHtml}</div>` : ''}
    <div style="display:flex;gap:8px;flex-wrap:wrap">
      ...
      ${hintsHtml ? `<button onclick="
        ...
        if(h){h.style.display=h.style.display==='none'?'flex':'none';this.textContent=h.style.display==='none'?'💡 Подробнее':'💡 Скрыть'}
      " style="background:rgba(255,255,255,.08);color:var(--fg);border:1px solid rgba(255,255,255,.15);border-radius:6px;padding:6px 12px;font-size:13px;cursor:pointer">💡 Подробнее</button>` : ''}
      <button onclick="localStorage.removeItem('msc_config');document.getElementById('msc-banner').remove()" style="
        background:none;border:1px solid rgba(255,255,255,.15);color:var(--fg2);
  ...
  document.getElementById('msc-apply-btn').addEventListener('click', function() {
    ...
    applyEntries.forEach(([id, val]) => {
      ...
      // Highlight element
      const wrap = el.closest('.field, .cb') || el;
      wrap.style.transition = 'box-shadow .15s, background .15s';
      wrap.style.boxShadow  = '0 0 0 2px #89dceb';
      wrap.style.background = 'rgba(137,220,235,.12)';
      wrap.style.borderRadius = '4px';
      setTimeout(() => { wrap.style.boxShadow=''; wrap.style.background=''; }, 2500);
      if (!firstEl) firstEl = wrap;

      const label = typeof val === 'boolean' ? (val ? '✅ вкл' : '❌ выкл') : `= ${val}`;
      changed.push(`<b>${id}</b> ${label}`);
    });
    ...
    // Update banner to show applied changes
    const b = document.getElementById('msc-banner');
    if (b) {
      b.innerHTML = `
        <div style="display:flex;justify-content:space-between;align-items:center;gap:10px">
          <div style="font-size:13px;font-weight:600;color:#a6e3a1">
            ✅ Применено — ${changed.length} изменений
            <span style="font-weight:400;color:var(--fg2);font-size:11px;margin-left:6px">${changed.join(' · ')}</span>
          </div>
          <button onclick="document.getElementById('msc-banner').remove()" style="
            background:none;border:none;color:var(--fg2);cursor:pointer;font-size:16px;padding:0;flex-shrink:0
          ">✕</button>
        </div>
        <div style="font-size:11px;color:var(--fg2)">Настройки подсвечены выше (синяя рамка). Прокрути страницу вверх чтобы увидеть их.</div>
      `;
      b.style.borderColor = 'rgba(166,227,161,.5)';
    }
  });
}
