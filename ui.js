...
function switchTableMode(mode) {
  ...
  if (oosBtn) oosBtn.style.display = DATA && DATA.length > 0 ? '' : 'none';
  // При переключении в режим HC — сбрасываем фильтры автоматически
  if (mode === 'hc') {
...
function resetAllFilters() {
  ...
  ['f_fav','f_split','f_ls','f_rob','f_oos','f_walk','f_param','f_noise','f_mc','f_oos_auto'].forEach(id => {
    const el = $(id); if (el) el.value = '';
  });
...
function renderVisibleResults() {
  ...
  for (let i = 0; i < page.length; i++) {
    ...
    html +=
      ...
      `<td class="col-oos-auto" title="IS/OOS">${oosCell}</td>` +
...
function showDetail(r) {
  ...
  function slName(pair) {
    ...
    if (pair.combo) {
      ...
      return `ATR ×${pair.a.m}  ${c.slLogic.toUpperCase()}  ${pair.p.m}%  [${lg}]`;
    }
    if (pair.a) return `ATR × ${pair.a.m}`;
    if (pair.p) return `${pair.p.m}% от цены`;
    return '—';
  }
  function tpName(pair) {
    ...
    function one(t) {
      if (!t) return '—';
      if (t.type==='rr') return `R:R = ${t.m} (от SL)`;
      if (t.type==='atr') return `ATR × ${t.m}`;
      return `${t.m}% от цены`;
    }
    if (pair.combo) {
  ...
  const _baseC = c.baseComm !== undefined ? c.baseComm : c.commission;
  const _spr   = c.spreadVal !== undefined ? c.spreadVal : 0;
...
function buildCopyText(r, c, slName, tpName) {
  ...
  if (r.wrL != null) lines.push('Лонг: WR ' + r.wrL.toFixed(0) + '% (' + r.nL + ' сд)   Шорт: WR ' + r.wrS.toFixed(0) + '% (' + r.nS + ' сд)   ΔWR L/S: ' + r.dwrLS.toFixed(0) + '%');
  lines.push('');
  ...
  lines.push('Безубыток:       ' + on(c.useBE,      'триггер=' + c.beTrig + 'xATR, оффсет=' + c.beOff + 'xATR'));
  lines.push('Trailing Stop:   ' + on(c.useTrail,   'триггер=' + c.trTrig + 'xATR, дист=' + c.trDist + 'xATR'));
  ...
  lines.push('Выход по времени:' + on(c.useTime,    'макс=' + c.timeBars + ' баров'));
  lines.push('Частичный TP1:   ' + on(c.usePartial, 'уровень=SLx' + c.partRR + ', закрыть ' + c.partPct + '%' + (c.partBE ? ', потом BE' : '')));
  ...
  lines.push('Свежесть тренда:  ' + on(c.useFresh,   'макс=' + c.freshMax + ' баров'));
  lines.push('Волатильность ATR:' + on(c.useVolF,    'ATR<' + c.volFMult + 'x среднего'));
  ...
  lines.push('Ликвидность:      ' + on(c.useLiq,     'мин=' + c.liqMin + 'x среднего'));
  lines.push('Направл. объёма:  ' + on(c.useVolDir,  'окно=' + c.volDirPeriod + ' баров'));
  lines.push('Взвеш. тренд WT:  ' + on(c.useWT,      'score>' + c.wtThresh + ', N=' + c.wtN + ', volW=' + c.wtVolW + ', bodyW=' + c.wtBodyW + (c.wtUseDist ? ', distMA=да' : '')));
  lines.push('Усталость тренда: ' + on(c.useFat,     c.fatConsec + ' свечей, vol<' + c.fatVolDrop + 'x'));
  lines.push('');
  lines.push('--- ОБЩЕЕ ---');
...
function loadFavAsTpl(i) {
  ...
  if (f.cfg) {
    ...
    const r = { name: f.name, cfg: f.cfg,
      pnl: f.stats.pnl, wr: f.stats.wr, n: f.stats.n,
      dd: f.stats.dd, pdd: f.stats.pdd, avg: f.stats.avg||0,
...
function _applyOOSHeaders(oosMode) {
  ...
  if (oosMode) {
    ...
    const nOld = _oosTableResults.length > 0 ? (_oosTableResults[0].old_bars || 0) : 0;
    const nNew = _oosTableResults.length > 0 ? (_oosTableResults[0].new_bars || 0) : 0;
    headerRow.innerHTML = `
      ...
      <th class="oos-th-hist" onclick="doOOSSort('old_pnl')" style="min-width:58px;resize:horizontal;overflow:auto" title="PnL IS — первые 70% (${nOld} баров)">PnL% IS ↕</th>
      <th class="oos-th-new"  onclick="doOOSSort('new_pnl')" style="min-width:58px;resize:horizontal;overflow:auto" title="PnL OOS — последние 30% (${nNew} баров)">PnL% OOS ↕</th>
      <th class="oos-th-delta" onclick="doOOSSort('delta_pnl')" style="min-width:54px;resize:horizontal;overflow:auto">ΔPnL ↕</th>
      <th class="oos-th-hist" onclick="doOOSSort('apt_old')" style="min-width:60px;resize:horizontal;overflow:auto" title="Ср. PnL% на сделку — IS (70%)">Avg/tr IS ↕</th>
      <th class="oos-th-new"  onclick="doOOSSort('apt_new')" style="min-width:60px;resize:horizontal;overflow:auto" title="Ср. PnL% на сделку — OOS (30%)">Avg/tr OOS ↕</th>
      <th class="oos-th-delta" onclick="doOOSSort('delta_apt')" style="min-width:56px;resize:horizontal;overflow:auto">ΔAvg/tr ↕</th>
      <th class="oos-th-hist" onclick="doOOSSort('old_wr')" style="min-width:50px;resize:horizontal;overflow:auto" title="WR — IS (70%)">WR% IS ↕</th>
      <th class="oos-th-new"  onclick="doOOSSort('new_wr')" style="min-width:50px;resize:horizontal;overflow:auto" title="WR — OOS (30%)">WR% OOS ↕</th>
...
function _getOOSSortVal(r, key) {
  ...
  if (key === 'apb_old') return (r.old_bars && r.old_pnl !== null) ? r.old_pnl / r.old_bars * 1000 : null;
  if (key === 'apb_new') return (r.new_bars && r.new_pnl !== null) ? r.new_pnl / r.new_bars * 1000 : null;
  if (key === 'delta_apb') {
    const ao = (r.old_bars && r.old_pnl !== null) ? r.old_pnl / r.old_bars * 1000 : null;
    const an = (r.new_bars && r.new_pnl !== null) ? r.new_pnl / r.new_bars * 1000 : null;
    return (ao !== null && an !== null) ? an - ao : null;
  }
  if (key === 'score') {
    if (r.new_pnl > 0) {
...
function _renderOOSSummary() {
  ...
  const stable  = _oosTableResults.filter(r => {
    const ao = r.old_n > 0 ? r.old_pnl / r.old_n : 0;
    const an = r.new_n > 0 ? r.new_pnl / r.new_n : 0;
    const apbO = r.old_bars ? r.old_pnl / r.old_bars : 0;
    const apbN = r.new_bars ? r.new_pnl / r.new_bars : 0;
  ...
  const oosBarCount = _oosTableResults.length > 0 ? (_oosTableResults[0].new_bars || 0) : 0;
  const newInfo = oosBarCount > 0 ? `OOS: ${oosBarCount} баров` : '';
...
// Запуск OOS-сравнения: IS = первые 70% DATA, OOS = последние 30% DATA
async function runOOSOnNewData() {
  if (!DATA || DATA.length < 20) { alert('Нет исходных данных'); return; }

  // Разбиваем текущие данные на IS (70%) и OOS (30%)
  const isN    = Math.floor(DATA.length * 0.70);
  const IS_DATA  = DATA.slice(0, isN);
  const OOS_DATA = DATA.slice(isN);
  ...
  for (let i = 0; i < srcList.length; i++) {
    ...
    try { DATA = IS_DATA;  rOld = _hcRunBacktest(r.cfg); } catch(e) {}
    try { DATA = OOS_DATA; rNew = _hcRunBacktest(r.cfg); } catch(e) {}
    DATA = origDATA;
    ...
    _oosTableResults.push({
      ...
      old_bars:  IS_DATA.length,
      new_bars:  OOS_DATA.length,
    });
  }
