...
function drawEquityData(eq, label, splitPct, baselineEq=null) {
  ...
  // 🔧 WAVE 11: Использовать пересчитанную baseline если она есть и включена
  // baselineEqExtended это Strategy Equity для полного 100% периода
  const shouldUseBaseline = baselineEqExtended && baselineEqExtended.length === eq.length && _eqMAFilterShowBaseline;
  if (shouldUseBaseline) {
    for(let i=0;i<baselineEqExtended.length;i++) {if(baselineEqExtended[i]<mn)mn=baselineEqExtended[i];if(baselineEqExtended[i]>mx)mx=baselineEqExtended[i];}
  }
  ...
  if (shouldUseBaseline) {
    ...
    const baselineNLast = Math.max(baselineEqExtended.length - 1, 1);
    ...
    for(let px=0;px<nPx;px++) {
      ...
      const y=H-pad-((baselineEqExtended[i]-mn)/range*(H-2*pad));
      if(px===0)ctx.moveTo(x,y);else ctx.lineTo(x,y);
    }
    ctx.stroke();
    ctx.globalAlpha = 1;
...
function drawEquityForResult(r) {
  ...
  // 🔧 WAVE 11: Если baselineEq короче чем основная eq - пересчитать для полных данных
  // Baseline создана на 70% IS периода, а нам нужна для 100% полного периода
  let baselineEqExtended = baselineEq;
  if (baselineEq && baselineEq.length < (eqToDisplay?.length || 10000) && r.cfg && r.cfg.useEqMA) {
    if (window.__DEBUG_EQUITY) console.log('  │  🔄 Recalculating baseline for full 100% period...');
    try {
      // Пересчитать Strategy Equity (без MA фильтра) для полных данных
      const cfgNoFilter = JSON.parse(JSON.stringify(r.cfg));
      cfgNoFilter.useEqMA = false;  // Отключаем MA фильтр чтобы получить чистую Strategy Equity
      const rawBacktest = _hcRunBacktest(cfgNoFilter);
      if (rawBacktest && rawBacktest.eq && rawBacktest.eq.length >= (eqToDisplay?.length || 10000)) {
        baselineEqExtended = Array.from(rawBacktest.eq);
        if (window.__DEBUG_EQUITY) console.log('  │  ✅ Baseline recalculated:', baselineEqExtended.length, 'bars');
      }
    } catch(e) {
      if (window.__DEBUG_EQUITY) console.log('  │  ❌ Baseline recalc failed:', e.message);
    }
  }
  ...
  if (window.__DEBUG_EQUITY) {
    ...
    console.log('  │  baselineEq (original):', baselineEq ? `array[${baselineEq.length}]` : 'NULL');
    console.log('  │  baselineEqExtended (WAVE 11 FIX):', baselineEqExtended ? `array[${baselineEqExtended.length}]` : 'NULL');
  }
  ...
  if (eqToDisplay && eqToDisplay.length) {
    ...
    drawEquityData(eqToDisplay, r.name, splitPct, baselineEqExtended);
