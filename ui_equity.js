...
function drawEquityData(eq, label, splitPct, baselineEq=null) {
  ...
  // 🔧 ВАЖНО: Baseline может быть другой длины (особенно с fallback к old_eq)
  // Это нормально - рисуем её как есть, без проверки длины
  // Масштабирование будет по обеим линиям автоматически
  const shouldUseBaseline = baselineEq && _eqMAFilterShowBaseline;
...
function drawEquityForResult(r) {
  ...
  if (window.__DEBUG_EQUITY) {
    ...
    const shouldUseBaseline = baselineEq && baselineEq.length === eqToDisplay.length && _eqMAFilterShowBaseline;
...
function _drawOOSGraphicForResult(r) {
  ...
  const baseline_old = r.old_eqCalcBaselineArr || r.old_eq; // baseline на истории (оранжевая) ##EQ_MA_FILTER## + fallback
  const baseline_new = r.new_eqCalcBaselineArr || r.new_eq; // baseline на новых данных (оранжевая) ##EQ_MA_FILTER## + fallback
