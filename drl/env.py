...
Действия:
  ...
  2 = EXIT  (выйти из позиции; игнорируется если флэт)

Награда:
  % доходность за бар (mark-to-market) пока в позиции
  -commission при входе и выходе
...
class TradingEnv(gym.Env):
    ...
    def __init__(self, df, commission=0.001, window=20):
        ...
        # Стартовый бар: нужно достаточно истории для window + прогрева RSI/ATR
    ...
    def step(self, action):
        ...
        # ── Выход ──
        elif action == 2 and self.in_trade:
            # Сначала mark-to-market до текущего close
            step_ret = (c - c_p) / max(c_p, 1e-8)
            self.equity *= (1.0 + step_ret) * (1.0 - self.commission)
            reward      += step_ret - self.commission
